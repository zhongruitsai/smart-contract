...
import { useWriteContract, useWaitForTransactionReceipt, useReadContracts, useBlock } from "wagmi";
...
function Step1Card({ pDeadline, vDeadline, nextProposalId, blockTimestamp, busy, onStartVoting, onClose }: {
  ...
  return (
    <div className="space-y-3">
      <InfoBox>
        股東可在提案截止日前提交提案。截止後，管理員需逐一為每個提案開放電子投票。
      </InfoBox>
      ...
      {!proposingOver && (
        <p className="text-xs text-amber-600 bg-amber-50 rounded p-2">
          ⚠ 提案期尚未截止，請使用底部時間控制器快轉至截止日後，再開放投票。
        </p>
      )}
      ...
      <button
        disabled={busy}
        onClick={onClose}
        className="w-full py-1.5 border border-red-300 text-red-500 rounded text-sm hover:bg-red-50 disabled:opacity-40 transition-colors"
      >
        關閉本次會議
      </button>
    </div>
...
function Step2Card({ nextProposalId, busy, onClose }: {
  nextProposalId: bigint | undefined;
  busy: boolean;
  onClose: () => void;
}) {
  ...
  return (
    <div className="space-y-3">
      <InfoBox>
        投票期間，股東可在各提案卡片上進行投票或委託。
        投票截止後，任何人可點擊提案卡片上的「結算提案」按鈕完成計票。
        所有提案結算完畢後，關閉本次會議。
      </InfoBox>
      <Stat label="進行中提案" value={`${count} 件`} />
      <p className="text-xs text-blue-600 bg-blue-50 rounded p-2">
        ℹ 股東頁面的提案列表中，投票截止後會出現「結算提案」按鈕。
      </p>
      <button
        disabled={busy}
        onClick={onClose}
...
function Step3Card({ onReopen, busy }: { onReopen: () => void; busy: boolean }) {
  return (
    <div className="space-y-3">
      <InfoBox>
        本次股東會已結束。所有提案結果可在股東頁面查閱。
        若需召開下次股東會，請點擊下方按鈕重新設立。
      </InfoBox>
      <ActionButton disabled={busy} onClick={onReopen}>
        {busy ? "送出中…" : "召開下次股東會 →"}
      </ActionButton>
    </div>
  );
...
function ActionButton({ children, disabled, onClick }: {
  ...
  return (
    <button
      disabled={disabled}
      onClick={onClick}
      className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium disabled:opacity-40 transition-colors"
    >
      {children}
    </button>
  );
...
export function PhaseController() {
  ...
  const { data, refetch } = useReadContracts({
    ...
    contracts: [
      ...
      { address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING, abi: GOVERNANCE_VOTING_ABI, functionName: "proposingDeadline" },
      { address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING, abi: GOVERNANCE_VOTING_ABI, functionName: "votingStartDeadline" },
      { address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING, abi: GOVERNANCE_VOTING_ABI, functionName: "nextProposalId" },
      { address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING, abi: GOVERNANCE_VOTING_ABI, functionName: "currentMeetingDate" },
    ],
  });
  ...
  useEffect(() => { if (isSuccess) { toast.success("操作成功"); refetch(); } }, [isSuccess]);

  function openPhase(meetingDateStr: string, isPublic: boolean, isExtraordinary: boolean) {
    writeContract({
      address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING,
      abi: GOVERNANCE_VOTING_ABI,
      functionName: "openProposingPhase",
      args: [dateToTs(meetingDateStr), isPublic, isExtraordinary],
    });
  }

  function startVoting(id: string, voteEndStr: string) {
    writeContract({
      address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING,
      abi: GOVERNANCE_VOTING_ABI,
      functionName: "startVoting",
      args: [BigInt(id || "0"), dateToTs(voteEndStr)],
    });
  }
  ...
  function closePhase() {
    writeContract({
      address: CONTRACT_ADDRESSES.GOVERNANCE_VOTING,
      abi: GOVERNANCE_VOTING_ABI,
      functionName: "closePhase",
      args: [],
    });
  }

  // Phase 3 (Closed) → reopen: just show Step0Card again
  ...
  return (
    <div className="border rounded-xl p-5 space-y-5 bg-white">
      ...
      <div className="border border-blue-100 rounded-lg p-4 bg-blue-50/30 space-y-3">
        {(phase === 0 || phase === 3) && (
          <Step0Card busy={busy} onSubmit={openPhase} />
        )}
        {phase === 1 && (
          <Step1Card
            pDeadline={pDeadline}
        ...
        {phase === 2 && (
          <Step2Card nextProposalId={nextProposalId} busy={busy} onClose={closePhase} />
        )}
        {phase === 3 && null}
      </div>
    </div>
